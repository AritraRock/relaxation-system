...
output_body = html.Div([
    dmc.Grid([
        html.Div([
            export_card
        ],style={'display': 'flex', 'justify-content':'space-around', 'align-item':'center','padding':'2'}),
        html.Div([
            dmc.LoadingOverlay(dmc.Image(id='model-output', height=380, width=1, style={'display':'none'}), loaderProps={"variant": "bars", "size": "xl"})
        ]),
        html.Div([
...
def show_output(n_clicks, contents, filename, start, duration, detectionCon, trackingCon, save_directory="./uploaded_videos"):
    if n_clicks and contents is not None:
        ...
        if video_path.lower().endswith('.webm'):
            mp4_path = video_path[:-5] + '.mp4'  # Change extension to .mp4
